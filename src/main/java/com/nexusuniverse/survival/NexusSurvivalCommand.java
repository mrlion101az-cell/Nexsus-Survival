...
public class NexusSurvivalCommand implements CommandExecutor {
    ...
    private void handleGive(Player player, String[] args) {
        if (args.length < 2) {
            player.sendMessage("§cUsage: /nexussurvival give <waterbottle|gasmask|wand|cure> [disease]");
            return;
        }
        switch (args[1].toLowerCase()) {
            ...
            default -> player.sendMessage("§cUnknown item. Options: waterbottle, gasmask, wand, cure");
        }
    }
