...
public class ThirstManager {
    ...
    private static final double DRINK_RESTORE = 6.0;
    private static final double RAW_DRINK_RESTORE = 3.0; // half -- untreated water isn't as reliably absorbed
    private static final int DECAY_INTERVAL_SECONDS = 45; // lose 1 thirst every 45s
    ...
    public void tick(Player player) {
        ...
        if (d.thirstTickCounter >= DECAY_INTERVAL_SECONDS) {
            d.thirstTickCounter = 0;
            d.thirst = Math.max(0, d.thirst - 1);
        }

        if (d.thirst <= 0) {
            d.thirstTickCounter++; // reuse counter's overflow while at 0 to pace damage
            if (d.thirstTickCounter % DEHYDRATION_DAMAGE_INTERVAL_SECONDS == 0) {
                player.damage(1.0);
    ...
    private void updateBar(Player player, SurvivalPlayerData d) {
        ...
        d.thirstBar.setColor(d.thirst <= 4 ? BarColor.RED : BarColor.BLUE);
    }
}
